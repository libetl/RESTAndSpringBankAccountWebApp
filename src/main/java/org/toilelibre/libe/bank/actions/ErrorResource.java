...
public class ErrorResource {
    ...
    public ObjectNode notFound () {
        ...
        return (ObjectNode) factory.objectNode ().put ("ok", 0).put ("name", "APINotFound").put ("description", "This API does not exist").put ("kind", Kind.BAD_INPUT.name ());
    }
    ...
    public ObjectNode badMethod () {
        final NodeFactory factory = NodeFactory.instance;
        return (ObjectNode) factory.objectNode ().put ("ok", 0).put ("name", "BadMethodOfAPI").put ("description", "This API exists, but the request method does not exist.").put ("kind",
                Kind.BAD_INPUT.name ());
    }
    ...
    public ObjectNode badEntityFormat () {
        final NodeFactory factory = NodeFactory.instance;
        return (ObjectNode) factory.objectNode ().put ("ok", 0).put ("name", "BadEntityFormat").put ("description", "This API does not understand this input format. It accepts JSON only.").put ("kind",
                Kind.BAD_INPUT.name ());
    }
    ...
    public ObjectNode internalServerError (final RuntimeException exception) {
        ...
        return (ObjectNode) factory.objectNode ().put ("ok", 0).put ("name", "InternalServerError").put ("description", "This API could not respond correctly. There was an API bug during the service")
    ...
    public ObjectNode handleApplicationException (final HttpServletRequest request, final HttpServletResponse response, final BankAccountException exception) {
        ...
        final NodeFactory factory = NodeFactory.instance;
        return (ObjectNode) factory.objectNode ().put ("ok", 0).put ("name", code.name ()).put ("description", code.getDescription ()).put ("kind", code.getKind ().name ());
    }
}
