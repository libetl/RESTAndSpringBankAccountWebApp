...
public class HtmlMessageConverter implements GenericHttpMessageConverter<Object> {
    ...
    public void write (final Object t, final Type type, final MediaType contentType, final HttpOutputMessage outputMessage) throws IOException, HttpMessageNotWritableException {
        ...
        Response<Object> response = (Response<Object>) t;
        outputMessage.getBody ().write (HEADER_WITH_DOCTYPE.getBytes ());
        ...
        Node content =  NodeFactory.instance.pojoNode (response.getContent ());
        
        ArrayNode links = new ArrayNode ();
        if (content instanceof ComplexObjectNode && 
                ((ComplexObjectNode<Node>)content).containsKey ("links")) {
    ...
    private void writeLinks (ArrayNode links, HttpOutputMessage outputMessage) throws IOException {
        if (links.size () == 0) {
            return;
        }
        ...
        for (Node link : links) {
            @SuppressWarnings ("unchecked")
            ComplexObjectNode<PrimitiveNode> trueLink = (ComplexObjectNode<PrimitiveNode>) link;
            outputMessage.getBody ().write (("<li><a href=\"" + trueLink.get ("href").asText () + "\" title=\"" + trueLink.get ("rel").asText () + "\">" +
                    trueLink.get ("href").asText () + "</a></li>").getBytes ());
        }
        outputMessage.getBody ().write ("</ul>".getBytes ());
    }

    private void writeContent (ComplexObjectNode<Node> content, HttpOutputMessage outputMessage, ArrayNode links) throws IOException {
        outputMessage.getBody ().write ("<ul>".getBytes ());
        ...
        for (Entry<String, Node> node : content.entrySet ()) {
    ...
    @SuppressWarnings ("unchecked")
    private void writeContent (Node value, HttpOutputMessage outputMessage, ArrayNode links) throws IOException {
    ...
    private void writeContent (ArrayNode value, HttpOutputMessage outputMessage, ArrayNode links) throws IOException {
    ...
    private void writeContent (PrimitiveNode value, HttpOutputMessage outputMessage) throws IOException {
        outputMessage.getBody ().write (("<code>" + value.asText () + "</code>").getBytes ());
    }

    private void writeDefaultResponse (HttpOutputMessage outputMessage, boolean alreadyWritten) throws IOException {
        if (!alreadyWritten) {
            outputMessage.getBody ().write ((HEADER_WITH_DOCTYPE + "Unrecognized response</title></head><body><h1>Problem</h1><p>Sorry, unrecognized response</p>" + FOOTER_WITH_GO_BACK_LINK).getBytes ());
        }
    }

    private boolean writeError (Object t, HttpOutputMessage outputMessage) throws IOException {
        if (t instanceof ObjectNode && "0".equals (((ObjectNode<?>)t).get ("ok").asText ())) {
            ObjectNode<?> errorPayload = (ObjectNode<?>) t;
            outputMessage.getBody ().write (
                    (HEADER_WITH_DOCTYPE + "Error</title></head><body><h1>" + errorPayload.get ("name").asText () + "</h1><p>The kind of this error is <code>" + 
                            errorPayload.get ("kind").asText () + "</code></p><p><u>Description :</u> " + errorPayload.get ("description").asText () + "</p>" + FOOTER_WITH_GO_BACK_LINK).getBytes ());            
            return true;
        }
        return false;
    }
